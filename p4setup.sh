...
set +x
echo "------------------------------------------------------------"
echo "Installing bmv2, needed for installing PI with bmv2"
echo "start install bmv2:"
set -x
date
...
sudo apt -y update
sudo apt -y install libnanomsg-dev p4lang-bmv2

set +x
echo "end install bmv2:"
set -x
date
...
git log -n 1
./autogen.sh
./configure --with-proto --with-internal-rpc --without-cli --with-bmv2
make
sudo make install
